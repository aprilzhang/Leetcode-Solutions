...
public class Solution {
    public int numTrees(int n) {
        return numTrees(new int[n+1][n+1], 1,n);
    }
    
    public int numTrees(int[][] cache, int lower,int upper)
    {
        if(lower>=upper)
        {
            return 1;
        ...
        {
            sum += numTrees(cache,lower,i-1)*numTrees(cache,i+1,upper);
        }
        cache[lower][upper] = sum;
        return sum;
    }
}
/**
