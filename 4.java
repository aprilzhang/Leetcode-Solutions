...
public class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        final int halfLength = (length1+length2+1)/2;
        
        //swap nums1 and nums2 to make sure nums1 is the shorter one.
        if(length1>length2)
        {
            int[] temp = nums1;
            nums1 = nums2;
        ...
        while(imin<=imax)
        {
            ...
            {
                final int max_of_left = i==0
                                        ?nums2[j-1]
                                        :j == 0
                                         ?nums1[i-1]
                                         :Math.max(nums1[i-1], nums2[j-1]);
                //Odd number of nums1, so return the last one.
                if ((length1 + length2) % 2 == 1)
                {
                    return max_of_left;
                }
                
                final int min_of_right = i==length1
                                        ?nums2[j]
                                        :j == length2
                                         ?nums1[i]
