...
public class Solution {
    public int maxCoins(int[] nums) {
        ...
        return burst(new int[nums.length+2][nums.length+2], balloons,0,nums.length+1);
    }
    
    public int burst(int[][] cache, int[] balloons, int left, int right) {
        if(left+1==right)
        {
            return 0;
        }
        ...
        {
            final int coins =balloons[left] * balloons[i] * balloons[right]
            + burst(cache, balloons, left, i) 
            + burst(cache, balloons, i, right);
